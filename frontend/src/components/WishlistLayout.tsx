...
export default function WishlistLayout({ 
  ...
  const [hasFetchedWishlist, setHasFetchedWishlist] = useState(false) // 👈 NEW

  useEffect(() => {
    const fetchWishlistImages = async () => {
      if (isLoadingWishlist || hasFetchedWishlist) return // 👈 prevents duplicate fetch
    ...
    fetchWishlistImages()
  }, [isLoadingWishlist, hasFetchedWishlist]) // 👈 add dependency
  ...
  const slots = useMemo(() => {
    ...
    const allImages = [...aestheticImages, ...wishlistImages]
    
    // Deduplicate by image URL to prevent same image appearing multiple times
    const seenUrls = new Set()
    const combinedImages = allImages.filter(img => {
